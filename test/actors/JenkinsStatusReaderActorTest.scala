...
import JenkinsStatusReaderCommands.{BuildsStatusSummary, ReadBuildsStatuses, RegisterObservedBuild}
import domain.{Build, BuildStatus, BuildIdentifier}
import support.ForwardingActorProps

class JenkinsStatusReaderActorTest extends TestKit(ActorSystem("test")) with WordSpec with ShouldMatchers {
  ...
  trait MockedJenkinsBuildActorFactory extends JenkinsBuildActorFactory {
    def newJenkinsBuildActor(buildIdentifier: BuildIdentifier) = ForwardingActorProps(childActor)
  }

  implicit val sender = testActor

  "Status Reader with registered builds" when {
    val actor = TestActorRef(new JenkinsStatusReaderActor with MockedJenkinsBuildActorFactory)
